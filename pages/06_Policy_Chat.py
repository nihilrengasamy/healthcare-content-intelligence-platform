"""Policy chat page with RAG integration."""
...
if not chunks:
    ...
    selected_chunks = selected_payload.get("chunks", []) if selected_payload else []
    ...
    with col1:
        if st.button("Build or Reuse Vector Index", type="primary"):
            if st.session_state.get("vector_store") and st.session_state.get("embeddings"):
    ...
    with col2:
        st.info("Embeddings are generated only for the selected active policy document.")

vector_store = st.session_state.get("vector_store")
if vector_store:
    render_metric_row(
        [
            {"title": "Chunks Indexed", "value": str(vector_store.get_index_statistics().get("chunks", 0)), "delta": "FAISS", "icon": "IDX", "color": "#1455a0"},
            {"title": "Embedding Dim", "value": str(vector_store.get_index_statistics().get("embedding_dimension", 0)), "delta": "Vectors", "icon": "VEC", "color": "#047857"},
            {"title": "Model", "value": str(vector_store.get_index_statistics().get("model", "")), "delta": "Embedding", "icon": "MOD", "color": "#7c3aed"},
        ]
    )
